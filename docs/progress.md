...
| 028 - Performance ingestion and review | Partial | Backend tests 15/15; backend ruff; frontend typecheck; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; credentialed provider sync, provider-specific schemas beyond generic JSON, model-backed extraction, and full verification remain. |
...
10. Add credentialed provider sync, provider-specific schemas beyond generic
   JSON/narrative parsers, and model-backed extraction accuracy evaluation.
