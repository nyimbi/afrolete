...
| Integrations and webhooks | partial | Keycloak OIDC bearer-token validation, frontend PKCE session handling, SpiceDB gRPC authorization adapter, shared OpenBao secret resolution with vault-path precedence, secret-safe infrastructure readiness reporting, live dependency probe reporting, tenant developer applications, sandbox/production API keys with persisted quota enforcement, SDK-facing API-key authorization for organization and training-drill routes, managed developer integration catalogs, webhook subscriptions with persisted delivery fanout, replay controls, retry-run controls, and marketplace listings are implemented; live service smoke tests, broader SDK route coverage, distributed quota counters, background retry scheduling/exponential backoff, OAuth consent, public developer docs, generated SDK packages, and other integrations remain future slices. |
