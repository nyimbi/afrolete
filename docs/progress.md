...
| 239 - Performance environmental injury-risk signals | Partial | Focused backend/frontend checks | Injury-risk scoring now uses linked training-session event weather assessments and venue facility surfaces, adding critical/warning/advisory weather penalties, hazardous-surface markers, environmental risk counts, weather decisions, surface driver text, and operations-console weather/surface bars; biomechanical/video signals, live model-provider inference, browser visual QA, and clinical validation remain. |
| 240 - Performance wearable biomarker injury-risk signals | Partial | Focused backend/frontend checks | Injury-risk scoring now consumes accepted wearable performance observations for HRV, resting heart rate, sleep duration/quality, recovery/readiness scores, hydration, strain, stress, and body temperature; the risk response includes wearable observation counts, biomarker risk counts, latest HRV/resting-HR/recovery/hydration values, wearable risk labels, v3 model-policy output, and operations-console biomarker/recovery bars; live wearable provider sync credentials, richer device-specific schemas, biomechanical/video signals, browser visual QA, and clinical validation remain. |
