...
| 231 - Regional performance benchmark cohorts | Partial | Focused backend/frontend checks | Performance benchmarks now support country/region cohort scopes using person `country_code`, member and athlete-intake creation can capture country codes for rostered athletes, and both the operations console and player portal expose country/region benchmark selectors; subnational/association-region cohorts, browser visual QA, and richer cohort comparison controls remain. |
| 232 - Performance cohort comparison dashboard | Partial | Focused backend/frontend checks | Athlete performance now exposes `/performance/athletes/{athlete_profile_id}/cohort-comparisons` with tenant, age-group, position, and region comparison summaries, embeds the same summaries in player profiles, and renders side-by-side comparison cards in the operations console and player portal; subnational/association-region cohorts, browser visual QA, and deeper comparison controls remain. |
