...
| 131 - Signed travel fee payment webhooks | Partial | Fast backend/frontend checks | Travel fee payment providers can now post provider-neutral settlement callbacks to a dedicated webhook endpoint with timestamped HMAC signature enforcement, configurable signing key/tolerance, duplicate external-reference protection through the settlement path, and signature-required/validated response metadata; live Stripe/M-Pesa/PayPal payload mapping, provider smoke tests, and browser payment QA remain. |
