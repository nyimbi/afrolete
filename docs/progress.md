...
| 260 - Player performance what-if forecasts | Partial | Focused backend/frontend checks | Player performance profiles now include deterministic what-if forecast scenarios, `/performance/my-profiles` accepts player-facing load/readiness/horizon controls, what-if scenarios honor the selected metric domain/code, and `/player` renders persisted what-if cards/runways beside baseline forecasts; browser visual QA with loaded profile data, live model-provider forecasts, and deeper chart interactions remain. |
