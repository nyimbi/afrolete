...
| 167 - AI scorecard artifact access analytics | Partial | Fast backend/frontend checks | Managers now get a derived scorecard artifact access summary with total events, link-created/opened counts, PDF/Markdown mix, unique requester count, latest access timestamp, and source buckets in the operations console; time-series charts, anomaly alerts, live MinIO smoke, and browser QA remain. |
