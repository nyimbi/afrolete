...
| 221 - Player self-assessment submission | Partial | Focused backend/frontend checks; migration authored | Athlete-linked accounts can now submit their own pending-review self-assessments through `/performance/my-profiles/{athlete_profile_id}/self-assessments`, including physical/technical/tactical/mental scores, RPE, effort rating, notes, and computed ALS; `/player` now includes a self-assessment form that refreshes the player's profile after submission; coach review workflows, browser QA, and full backend tests remain. |
