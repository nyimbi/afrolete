...
| 028 - Performance ingestion and review | Partial | Backend tests 19/19; backend ruff; frontend typecheck; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; live OAuth token exchange/pull adapters, more vendor schemas, model-backed extraction, and full verification remain. |
...
10. Add live OAuth authorization/token refresh, provider pull adapters, more
   provider-specific schemas, provider webhook registration smoke tests, and
