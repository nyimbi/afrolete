...
| Consent, safeguarding, compliance, incidents | partial | Guardian relationships, consent requests, one-use web links, SMS/WhatsApp/Telegram/email/manual consent capture, family portal consent responses, minor event clearance, background-check tracking, compliance credential renewal tracking, and operator incident reporting/triage are implemented; real screening-provider integrations, investigation workflows, regulatory submission packages, automated expiry jobs, and advanced compliance dashboards remain. |
