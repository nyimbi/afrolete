...
| 028 - Performance ingestion and review | Partial | Backend tests 21/21; backend ruff; frontend typecheck/build; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, OAuth authorization/callback state handling, token lifecycle auditing, configurable provider pull adapters, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; more vendor schemas, model-backed extraction, browser QA, and full verification remain. |
...
| 247 - Performance wearable provider pull adapters | Partial | Focused backend/frontend checks; migration authored | Wearable sync now supports configured provider pull URLs with OpenBao/env-resolved bearer tokens, athlete/cursor/since/until query parameters, provider response hashing, HTTP status auditing, next-cursor persistence, replay-safe ingestion through the existing wearable normalizer, focused live-adapter unit coverage, and operations-console Pull controls; provider-specific pagination/rate-limit adapters, browser visual QA, and live provider smoke tests remain. |
...
10. Add provider-specific pagination/rate-limit adapters, more provider-specific
   schemas, provider webhook registration smoke tests, browser visual QA, live
   wearable provider smoke tests, and model-backed extraction accuracy
   evaluation.
