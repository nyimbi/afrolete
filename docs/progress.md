...
| 028 - Performance ingestion and review | Partial | Backend tests 26/26; backend ruff; frontend typecheck/build; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit/Polar/Oura/Catapult-style schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, OAuth authorization/callback state handling, token lifecycle auditing, configurable provider pull adapters with pagination/rate-limit audit, provider webhook registration auditing, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; model-backed extraction, browser QA, and full verification remain. |
...
| 241 - Performance provider evidence parsers | Partial | Focused backend/frontend checks | Performance ingestion now parses structured provider payloads from wearable/stat feeds by matching metric code/name/single-metric payloads, extracts metric-specific values from video/audio/text narratives instead of blindly using the first number, preserves provider timestamps, returns parser method/confidence reason/warnings/parsed fields, and surfaces parser metadata in the operations console; live provider credentialed sync, model-backed extraction accuracy, browser visual QA, and full verification remain. |
...
| 252 - Performance wearable provider schema expansion | Partial | Focused backend/frontend checks | Wearable ingestion now normalizes Polar heart-rate/HRV/sleep summaries, Oura readiness/sleep summaries, and Catapult/STATSports/Playertek GPS workload metrics such as player load, total distance, max speed, high-speed distance, accelerations, decelerations, and average heart rate; the operations console provider selector and sample webhook payloads now include these providers; live provider smoke tests, browser visual QA, and model-backed extraction evaluation remain. |
...
10. Add provider webhook registration smoke tests, browser visual QA, live
   ...
   tests, and model-backed extraction accuracy evaluation.
