...
| 241 - Performance provider evidence parsers | Partial | Focused backend/frontend checks | Performance ingestion now parses structured provider payloads from wearable/stat feeds by matching metric code/name/single-metric payloads, extracts metric-specific values from video/audio/text narratives instead of blindly using the first number, preserves provider timestamps, can apply deterministic or webhook-backed model assistance to weak narrative parses, returns parser/model method/confidence reason/warnings/parsed fields, and surfaces parser metadata in the operations console; live provider credentialed sync, model accuracy benchmark suites, browser visual QA, and full verification remain. |
...
| 252 - Performance wearable provider schema expansion | Partial | Focused backend/frontend checks | Wearable ingestion now normalizes Polar heart-rate/HRV/sleep summaries, Oura readiness/sleep summaries, and Catapult/STATSports/Playertek GPS workload metrics such as player load, total distance, max speed, high-speed distance, accelerations, decelerations, and average heart rate; the operations console provider selector and sample webhook payloads now include these providers; live provider smoke tests, browser visual QA, and model accuracy benchmark suites remain. |
| 253 - Performance model-assisted extraction boundary | Partial | Focused backend/frontend checks | Performance ingestion now has a governed model extraction boundary with deterministic local mode, optional signed webhook mode, model policy/configuration fields, model confidence and evaluation metadata in API responses, and operations-console visibility; model assistance only replaces weak parser fallbacks and preserves precise structured or metric-specific parser matches; model accuracy benchmark suites, live model-provider smoke tests, and browser visual QA remain. |
...
10. Add provider webhook registration smoke tests, browser visual QA, live
   ...
   tests, and model accuracy benchmark suites.
