...
| 028 - Performance ingestion and review | Partial | Backend tests 18/18; backend ruff; frontend typecheck; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; credentialed OAuth provider sync, more vendor schemas, model-backed extraction, and full verification remain. |
...
10. Add credentialed provider sync, more provider-specific schemas, provider
   webhook registration, live provider smoke tests, and model-backed extraction
   accuracy evaluation.
