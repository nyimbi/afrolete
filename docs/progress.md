...
| 220 - Player performance self-service portal | Partial | Focused backend/frontend checks | Athlete-linked accounts can now load their own organization performance profile through `/performance/my-profiles`, including ALS summary, recent observations, goals, awards, filtered trends, trend-series history, forecast scenarios, cohort comparisons, and cohort benchmarks; the new `/player` portal presents that data in a responsive self-service dashboard with local identity controls; richer charts, browser QA, and full backend tests remain. |
