...
| Integrations and webhooks | partial | Keycloak OIDC bearer-token validation, frontend PKCE session handling, SpiceDB gRPC authorization adapter, shared OpenBao secret resolution with vault-path precedence, secret-safe infrastructure readiness reporting, live dependency probe reporting, tenant developer applications, sandbox/production API keys with persisted quota enforcement, OAuth-style developer consent grants with PKCE public-client exchange, refresh-token rotation, refresh-family compromise detection, replay incident automation, SDK-facing API-key authorization for organization, teams, events, and training-drill routes, managed developer integration catalogs, public developer docs, TypeScript SDK package, webhook subscriptions with persisted delivery fanout, replay controls, retry-run controls with exponential backoff, retry worker CLI, unified due-worker lane support, systemd due-worker scheduling, and marketplace listings are implemented; live service smoke tests, broader SDK write coverage, distributed quota counters, SDK package publishing/generated clients, Python SDK implementation, and other integrations remain future slices. |
