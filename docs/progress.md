...
| 103 - Travel carpool auto-matching | Partial | Fast backend/frontend checks | Travel carpool requests and offers can now be automatically matched from existing ride records using seat capacity, pickup/dropoff token overlap, and departure-window compatibility, update request/offer statuses with deterministic match scores, and expose auto-match controls/results in the console; geocoding, map-distance scoring, driver ratings, and backup driver networks remain. |
