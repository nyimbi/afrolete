...
| 199 - SDK event route coverage | Partial | Fast backend checks | Tenant developer API keys can now list and create events through `/api/v1/sdk/events` with event-scope enforcement, team validation, active `events.created` webhook fanout, catalog updates, SDK entry-point metadata, public docs quickstart coverage, and TypeScript SDK wrappers; broader SDK route coverage, SDK package publishing/generated clients, and browser QA remain. |
