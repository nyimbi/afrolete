...
| 028 - Performance ingestion and review | Partial | Backend tests 20/20; backend ruff; frontend typecheck; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, OAuth authorization/callback state handling, token lifecycle auditing, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; provider pull adapters, more vendor schemas, model-backed extraction, browser QA, and full verification remain. |
...
| 244 - Performance wearable provider connections and sync runs | Partial | Focused backend/frontend checks; migration authored | Operators can now persist athlete-scoped wearable provider connections with provider, external athlete reference, OAuth/OpenBao secret paths, scopes, webhook-registration state, sync cursor, default metric mappings, and token expiry metadata; connection sync runs can ingest sample provider payloads through the same replay-protected wearable pipeline, persist run status/counts/messages, list connection history, and expose Connect/Sync controls in the operations console; provider pull adapters, browser visual QA, and live provider smoke tests remain. |
...
10. Add provider pull adapters, more provider-specific schemas, provider webhook
   registration smoke tests, browser visual QA, live wearable provider smoke
