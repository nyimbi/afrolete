...
| Integrations and webhooks | partial | Keycloak OIDC bearer-token validation, frontend PKCE session handling, SpiceDB gRPC authorization adapter, shared OpenBao secret resolution with vault-path precedence, secret-safe infrastructure readiness reporting, live dependency probe reporting, tenant developer applications, webhook subscriptions, and marketplace listings are implemented; live service smoke tests, live webhook delivery fanout, OAuth consent, public developer docs, and other integrations remain future slices. |
