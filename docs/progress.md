...
| 028 - Performance ingestion and review | Partial | Backend tests 20/20; backend ruff; frontend typecheck; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, OAuth authorization/callback state handling, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; live provider token exchange/pull adapters, more vendor schemas, model-backed extraction, and full verification remain. |
...
10. Add live provider token endpoint exchange, refresh-token rotation, provider
   pull adapters, more provider-specific schemas, provider webhook registration
   smoke tests, and model-backed extraction accuracy evaluation.
