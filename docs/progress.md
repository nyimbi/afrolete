...
| 130 - Hosted travel fee payment page | Partial | Fast backend/frontend checks | Travel fee checkout sessions now resolve to a public hosted payment-page payload, render a dedicated `/pay/sessions/{sessionId}` portal with invoice status, due dates, method selection, outstanding totals, and confirmation controls, and accept provider-neutral settlement events that record finance payments and mark invoices partial/paid; live processor redirects, signed provider webhooks, PCI payment fields, and browser payment QA remain. |
