...
| 253 - Performance model-assisted extraction boundary | Partial | Focused backend/frontend checks | Performance ingestion now has a governed model extraction boundary with deterministic local mode, optional signed webhook mode, model policy/configuration fields, model confidence and evaluation metadata in API responses, and operations-console visibility; model assistance only replaces weak parser fallbacks and preserves precise structured or metric-specific parser matches; custom production benchmark datasets, live model-provider smoke tests, and browser visual QA remain. |
| 254 - Performance model extraction benchmark suites | Partial | Focused backend/frontend checks | Operators can now run `/performance/model-extraction/benchmarks` to evaluate parser and model-assisted extraction against default or supplied benchmark cases without creating observations, returning model policy, pass/fail counts, accuracy, mean absolute error, per-case parser methods, confidence, and summaries; the operations console includes a Benchmark action and result card; custom production benchmark datasets, live model-provider smoke tests, and browser visual QA remain. |
...
10. Add provider webhook registration smoke tests, browser visual QA, live
   ...
   tests, and custom production model benchmark datasets.
