...
| 209 - SDK roster assignment route | Partial | Focused compile/type checks | Developer API keys with `write:roster` or `write:teams` can now assign existing people to team rosters through `/api/v1/sdk/teams/{team_id}/members`, creating athlete profiles when needed, preserving team-person authorization relationships, and exposing roster assignment helpers in both SDKs; person creation SDK routes, guardian-aware minor onboarding, generated SDK clients, browser QA, and full backend tests remain. |
