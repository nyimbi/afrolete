...
| 288 - Signed incident regulatory artifact links | Complete | Focused safeguarding tests; frontend typecheck; backend lint | Incident regulatory report packages can now mint expiring signed Markdown/PDF links with HMAC signatures from env or OpenBao-backed keys, stable generation timestamps, checksum headers, public read routes, and operations-console Link controls; persisted object storage, richer branded layouts, electronic government portal submission, and insurer/medical portal integrations remain. |
...
| Consent, safeguarding, compliance, incidents | partial | Guardian relationships, SDK guardian linking, SDK consent request creation, consent requests, one-use web links, SMS/WhatsApp/Telegram/email/manual consent capture, family portal consent responses, minor event clearance, event travel consent request generation and reminders, travel safety manifests with guardian and medical clearance context, background-check tracking, provider-sourced screening risk/status intake, compliance credential renewal tracking, derived compliance dashboards, manual and scheduled expiry reconciliation, blocker/renewal/investigation queues, operator incident reporting/triage, developer OAuth replay security incidents, emergency activation incident generation, regulatory report packages with Markdown/PDF exports and signed artifact links, incident insurance claim tracking, return-to-play medical clearance, and medical-clearance-aware attendance gating are implemented; provider-specific screening submission adapters, deeper investigation workflows, electronic government portal submission, insurer API submission/status polling, medical portal integrations, richer attendance policy configuration, persisted document object storage, and access-control automation remain. |
