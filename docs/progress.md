...
| 236 - Performance injury risk alerts | Partial | Focused backend/frontend checks | High-risk athlete profiles can now trigger `/performance/athletes/{athlete_profile_id}/injury-risk/alerts`, which recomputes deterministic injury risk, checks a threshold, selects coaches/managers plus the athlete and consent-signing guardians, sends urgent in-app alert messages, suppresses duplicate recent alert clicks, and exposes an operations-console Send risk alert control with delivery feedback; provider push/SMS/WhatsApp fanout, browser visual QA, and live clinical/model validation remain. |
| 237 - Performance injury risk alert scans | Partial | Focused backend/frontend checks | Organization-wide injury-risk alert scans now expose `/performance/injury-risk/alert-scans` and a unified due-worker `performance-injury-risk-alerts` lane, scan athlete profiles by recent performance/readiness evidence, threshold deterministic injury risk, send urgent in-app alerts, report high-risk/skipped/failed counts, and apply repeat-after duplicate suppression; cron/systemd scheduling in deployment, provider push/SMS/WhatsApp fanout, browser visual QA, and clinical/model validation remain. |
