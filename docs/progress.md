...
| 028 - Performance ingestion and review | Partial | Backend tests 23/23; backend ruff; frontend typecheck/build; migration graph check; diff check | Provider-neutral evidence ingestion, structured provider payload parsing, WHOOP/Garmin/Apple Health/Fitbit schema normalization, signed wearable webhook ingestion with replay protection, wearable provider connection records, OAuth authorization/callback state handling, token lifecycle auditing, configurable provider pull adapters with pagination/rate-limit audit, provider webhook registration auditing, sync-run auditing, metric-specific narrative parsing, parser confidence/warning metadata, pending-review observations, human review/correction, and console controls are implemented; more vendor schemas, model-backed extraction, browser QA, and full verification remain. |
...
10. Add more provider-specific schemas, provider webhook registration smoke
   tests, browser visual QA, live wearable provider smoke tests, live provider
   webhook registration smoke tests, and model-backed extraction accuracy
   evaluation.
