...
| Integrations and webhooks | foundation | Keycloak OIDC bearer-token validation, frontend PKCE session handling, SpiceDB gRPC authorization adapter, and shared OpenBao secret resolution with vault-path precedence are implemented; live service smoke tests and other integrations remain future slices. |
