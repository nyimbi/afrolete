...
| Equipment, facilities, assets | partial | Facility profiles, equipment inventory, checkout/return, maintenance work orders, booking overlap checks, asset readiness metrics, scan lookup, RFID scan intake/history, photo metadata, equipment file uploads, procurement recommendations, supplier scorecards, supplier orders/receiving/submission, lease quotes and lease invoice creation, utilization recommendations, and console workflows are implemented; MinIO persistence, live RFID reader gateway provisioning, supplier-specific adapters, invoice sync, accounting API sync, and lease payment schedules remain. |
...
4. Add MinIO-backed equipment storage, live RFID reader gateway provisioning,
   supplier-specific adapters, supplier invoice sync, accounting API sync, and
   lease payment schedules for assets and facilities.
