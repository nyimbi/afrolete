...
| 121 - Mobile travel manifest offline cache | Partial | Fast frontend typecheck; diff check | The operations console can now cache a fetched travel manifest into browser storage, restore it later without a network call, show cache timestamp/version metadata, and expose Cache/Offline controls for field/mobile use; encrypted offline storage, service-worker prefetch, cache expiry policy, and browser QA remain. |
