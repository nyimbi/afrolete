...
| Integrations and webhooks | foundation | Keycloak OIDC bearer-token validation, frontend PKCE session handling, SpiceDB gRPC authorization adapter, shared OpenBao secret resolution with vault-path precedence, secret-safe infrastructure readiness reporting, and live dependency probe reporting are implemented; live service smoke tests and other integrations remain future slices. |
