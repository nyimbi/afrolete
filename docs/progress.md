...
| 192 - Developer integration catalog | Partial | Fast backend/frontend checks | Tenant operators now get a managed developer catalog for API auth headers, SDK entry points, scope descriptions, configured webhook event types, active/cataloged event payload contracts, webhook signing metadata, console cards, and focused test coverage; public hosted docs, generated SDK packages, OAuth consent, and browser QA remain. |
| 193 - Public developer docs portal | Partial | Fast backend/frontend checks; frontend build | AfroLete now exposes public developer documentation at `/developers` backed by `/api/v1/developers/public/docs`, including quickstarts, auth headers, SDK surfaces, marketplace lanes, webhook contracts, signature guidance, security requirements, and console navigation; generated SDK packages, searchable docs, OAuth consent, and browser QA remain. |
...
| 199 - SDK event route coverage | Partial | Fast backend checks | Tenant developer API keys can now list and create events through `/api/v1/sdk/events` with event-scope enforcement, team validation, active `events.created` webhook fanout, catalog updates, SDK entry-point metadata, and public docs quickstart coverage; broader SDK route coverage, generated SDK packages, and browser QA remain. |
