...
class Event(IdMixin, TimestampMixin, Base):
    ...
    team_id: Mapped[str | None] = mapped_column(GUID(), ForeignKey("teams.id"), index=True)
...
class AttendanceRecord(IdMixin, TimestampMixin, Base):
    ...
    event_id: Mapped[str] = mapped_column(GUID(), ForeignKey("events.id"), index=True)
    ...
    recorded_by_person_id: Mapped[str | None] = mapped_column(GUID(), ForeignKey("persons.id"))
