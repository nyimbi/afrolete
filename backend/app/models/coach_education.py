...
class CoachEducationEnrollment(IdMixin, TimestampMixin, Base):
    ...
    accreditation_provider: Mapped[str | None] = mapped_column(String(180), index=True)
    certificate_number: Mapped[str | None] = mapped_column(String(120), index=True)
    certification_issued_on: Mapped[date | None] = mapped_column(Date, index=True)
    certification_expires_on: Mapped[date | None] = mapped_column(Date, index=True)
    renewal_due_on: Mapped[date | None] = mapped_column(Date, index=True)
    cpd_hours_required: Mapped[float] = mapped_column(Float, default=20.0, nullable=False)
    cpd_hours_completed: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    portfolio_evidence_ref: Mapped[str | None] = mapped_column(String(500))
    mentor_person_id: Mapped[UUID | None] = mapped_column(GUID(), ForeignKey("persons.id"), index=True)
...
class CoachEducationActivity(IdMixin, TimestampMixin, Base):
    ...
    cpd_hours: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    reviewer_person_id: Mapped[UUID | None] = mapped_column(GUID(), ForeignKey("persons.id"), index=True)
    review_status: Mapped[str] = mapped_column(String(40), default="accepted", nullable=False, index=True)
